...
container_node* CreateTabWindow(menu_interface* Interface)
{
  ...
  container_node* TabItemCollection = ConnectNodeToBack(TabWindowHeader, NewContainer(Interface));
  TabItemCollection->StackHorizontal = true;
...
internal container_node* GetTabGridFromWindow(container_node* TabbedWindow)
{
  ...
  Assert(TabHeader->Type == container_type::None);
  return TabHeader;
}
